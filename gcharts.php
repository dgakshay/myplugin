...
class Gcharts 
{
	...
	public $newplugin_type 		= 'LineChart'; //	LineChart, PieChart, ColumnChart, AreaChart, TreeMap, ScatterChart, Gauge, GeoChart, ComboChart, BarChart, CandlestickChart, Table
	...
	public function load($array = array())
	{
		...
		foreach(array('library_loaded','newplugin_type','create_div','dashboard_div','filter_div','chart_div','class_filter_div','class_dashboard_div','class_chart_div','open_js_tag','control_type') as $p)
		{
			if(isset($array[$p]))
			{
				if($p == 'newplugin_type')
				{
					$this->set_newplugin_type($array[$p]);
					continue;
				}
				$this->$p = $array[$p];
			}
	...
	public function set_newplugin_type($type = NULL)
	{
		...
		$this->newplugin_type = $types[$type];
		return true;
	}
	...
	private function GenWithoutDashboard($data)
	{
		...
		$js .= "var chart = new google.visualization.".$this->newplugin_type."(document.getElementById('".$this->chart_div."'));\n";
